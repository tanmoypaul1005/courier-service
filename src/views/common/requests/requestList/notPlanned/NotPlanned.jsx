...
export default function NotPlanned({ tableTitleClassName }) {
  ...
  const headers = [
    ...
    (user_role === role.customer) ?
      { isActive: requests_order === k_orderBy.awarded_company, sort: is_asc, index: 1, name: t("Company"), onClickAction: () => handleRequestOrder(k_orderBy.awarded_company, getData), width: '16' }
      : { isActive: requests_order === k_orderBy.customer_from, sort: is_asc, index: 1, name: t("Customer"), onClickAction: () => handleRequestOrder(k_orderBy.customer_from, getData), width: '16' },
    { isActive: requests_order === k_orderBy.title, sort: is_asc, index: 2, name: t("Title"), onClickAction: () => handleRequestOrder(k_orderBy.title, getData), width: '16' },
    { isActive: requests_order === k_orderBy.transport_type, sort: is_asc, index: 3, name: t("Type"), onClickAction: () => handleRequestOrder(k_orderBy.transport_type, getData), width: '16' },
    ...
    { isActive: requests_order === k_orderBy.pickup_starts_at, sort: is_asc, index: 5, name: t("Date & Time"), onClickAction: () => handleRequestOrder(k_orderBy.pickup_starts_at, getData), width: '16' },
    { isActive: requests_order === k_orderBy.awarded_budget, sort: is_asc, index: 6, name: t("Budget"), onClickAction: () => handleRequestOrder(k_orderBy.awarded_budget, getData), width: '16' },
  ];
  ...
  const resetTable = async () => {
    ...
    getTableViewRequestsData({ data_set: k_data_set.awarded, filter: { is_planned: user_role === role.company ? 0 : null } });
  }

  const handleInitialSetup = async () => {
    setTimeout(async () => {
      if (!path_record?.old?.includes('/requests/not-planned/details/')) {
        resetTable();
      } else {
        getData();
      }
      ...
      <CommonTable
        ...
        items={
          awarded?.data?.map((item, index) =>
            <tr className='border border-collapse hover:bg-cCommonListBG cp' onClick={() => navigateTo(`/requests/not-planned/details/${item?.id}`)}>
              <td className='border-r text-center py-2 table-title'>{(awarded?.current_page * 10 - 10) + index + 1}</td>
              {(user_role === role.customer) ?
                <td className='border-r text-center py-2 table-info'><Clamp lines={2}> {item?.awarded_company ?? 'NA'} </Clamp> </td>
                :
                <td className='border-r text-center py-2 table-info'><Clamp lines={2}> {item?.customer_from ?? 'NA'} </Clamp> </td>}
              <td className='border-r text-center py-2 table-info max-w-[180px] min-w-[180px]'><Clamp lines={2}> {item?.title ?? 'NA'} </Clamp> </td>
              ...
              <td className='border-r text-center py-2 table-info'>DKK {item?.awarded_budget?.toLocaleString("da-DK") ?? 'NA'}</td>
            </tr>
          )
        }
      />
